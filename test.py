if __name__ == "__main__":
    ...
    label3 = Label("This text is right-aligned") >> Padding(hpad=1) >> Border(single_line, fg_colour=red) >> Fill(bg_colour=dark_amber)
    label4 = Text('The standard chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested.\n\nSections 1.10.32 and 1.10.33 from "de Finibus Bonorum et Malorum" by Cicero are also reproduced in their exact original form, accompanied by English versions from the 1914 translation by H. Rackham.', 30) >> Padding(hpad=2, vpad=1) >> Border(single_line, fg_colour=red) >> Fill(bg_colour=dark_amber)
